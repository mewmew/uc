...
functor AbsynFn(Source : SOURCE) : ABSYN =
  struct
    ...
    structure IdentDict =
      AADictFn(struct
		 type ord_key = ident
		 fun compare(IDENT(s1,_,_), IDENT(s2,_,_)) =
		   String.compare(s1, s2)
	       end)

    fun makeIdent(name,left,right) = IDENT(name,left,right)
    fun identName(IDENT(name,_,_)) = name
    fun identEqual(IDENT(s1,_,_), IDENT(s2,_,_)) = s1=s2
